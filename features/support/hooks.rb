# frozen_string_literal: true

#Arquivo auxiliar para o funcionamento dos testes

#Função padrão do Capybara para iniciar antes de qualquer coisa
Before do
    @AddCarrinho = AdicionarCarrinho.new
    @pageCheckout = PaginaChekout.new
    @cadastroPage = CadastroPage.new
    @botao = Botoes.new
    @alertas = Alertas.new
    @Finalizar = FinzalizarCompra.new
end
  
  #Função padrão do Capybara para rodar após a cada cenário de teste
  After do |scenario|
    add_screenshot(scenario)
  
    add_browser_logs if scenario.failed?
  end
  
  
  #Metodo que tira um Screenshot(print) da tela após cada cenário de teste
  def add_screenshot(scenario)
    nome_cenario = scenario.name.gsub(/[^A-Za-z0-9]/, '')
    nome_cenario = nome_cenario.gsub(' ', '_').downcase!
    screenshot = "log/screenshots/#{nome_cenario}.png"
    page.save_screenshot(screenshot)
    embed(screenshot, 'image/png', 'Print maroto :)')
  end
  
  #metodo usado para gerar logs do projeto contendo informações do tipo: data, erros, acertos etc
  def add_browser_logs
    time_now = Time.now
    # Getting current URL
    current_url = Capybara.current_url.to_s
    # Gather browser logs
    logs = page.driver.browser.manage.logs.get(:browser).map { |line| [line.level, line.message] }
    # Remove warnings and info messages
    logs.reject! { |line| %w[WARNING INFO].include?(line.first) }
    logs.any? == true
    embed(time_now.strftime('%Y-%m-%d-%H-%M-%S' + "\n") + ('Current URL: ' + current_url + "\n") + logs.join("\n"), 'text/plain', 'BROWSER ERROR')
  end
  
  #metodo padrao do report builder utilizado para gerar relatórios html do projeto
  at_exit do
    time = Time.now.getutc
    ReportBuilder.configure do |config|
      config.json_path = 'report.json'
      config.report_path = 'cucumber_web_report'
      config.report_types = [:html]
      config.report_tabs = %w[Overview Features Scenarios Errors]
      config.report_title = 'Cucumber Report Builder web automation test results'
      config.compress_images = false
      config.additional_info = { 'Project name' => 'Test', 'Platform' => 'Integration', 'Report generated' => time }
    end
    ReportBuilder.build_report
  end
  
  
  